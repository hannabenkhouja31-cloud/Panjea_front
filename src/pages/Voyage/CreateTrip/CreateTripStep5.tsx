...
export const CreateTripStep5 = (props: CreateTripStep5Props) => {
    ...
    return (
        <div class="w-full max-w-3xl mx-auto p-2 sm:p-6">
            <div class="flex items-center justify-between mb-4 sm:mb-8">
                <div class="flex items-center gap-2 sm:gap-3">
                    <Camera class="w-6 h-6 sm:w-8 sm:h-8 text-color-main" />
                    <h3 class="text-xl sm:text-2xl font-bold text-color-dark">Photos du voyage</h3>
                </div>
                <span class="text-sm sm:text-base text-gray-500 font-medium">Étape {props.currentStep}/5</span>
            </div>

            <div>
                <h4 class="text-lg sm:text-xl font-bold text-color-dark mb-2">
                ...
                <Show when={props.temporaryMedia.length === 0}>
                    <label class="border-4 border-dashed border-gray-300 rounded-2xl p-6 sm:p-12 bg-gray-50 hover:bg-gray-100 hover:border-color-main transition-all cursor-pointer flex flex-col items-center justify-center">
                        <Camera class="w-12 h-12 sm:w-16 sm:h-16 text-gray-300 mb-4" />
                        <p class="text-base sm:text-lg font-semibold text-gray-600 mb-2 text-center">
                ...
                <Show when={props.temporaryMedia.length > 0}>
                            ...
                            {(media) => (
                                <div class="relative group aspect-square rounded-xl overflow-hidden border-2 border-gray-200 shadow-md">
                                    ...
                                    <button
                                        ...
                                        class="absolute top-2 right-2 opacity-100 sm:opacity-0 group-hover:opacity-100 bg-red-500 text-white p-1.5 sm:p-2 rounded-full hover:bg-red-600 hover:scale-110 transition-all shadow-lg z-10"
                                    >
                    ...
                    <Show when={props.temporaryMedia.length < 10}>
                        <label class="w-full py-3 sm:py-4 px-4 sm:px-6 border-2 border-color-main rounded-xl font-semibold text-color-main cursor-pointer hover:bg-color-main hover:text-white transition-all flex items-center justify-center gap-2 sm:gap-3 text-sm sm:text-base">
