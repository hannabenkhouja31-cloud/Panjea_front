        ...
        <div class="text-black border-l-2 sm:border-l-3 border-[#DC9E53] p-4 sm:p-8">
            <div class="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                <div class="p-1.5 sm:p-2 bg-color-secondary rounded-lg backdrop-blur-sm">
                    <Languages size={20} class="sm:w-6 sm:h-6" color="white" stroke-width={2.5}/>
                </div>
                <h3 class="text-lg sm:text-xl font-bold text-black">Langues parlées</h3>
            </div>
            <Show when={!props.isEditing}
                          ...
                          <div class="flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4 flex-wrap">
                                  ...
                                  {(lang, index) => (
                                      <div class="flex items-center gap-1">
                                          <button
                                              ...
                                              class={`cursor-pointer flex items-center gap-1.5 sm:gap-2 px-2 py-1.5 sm:px-3 rounded-lg font-semibold transition text-xs sm:text-base ${
                                          ...
                                          {props.editLanguages.length > 1 && (
                                              <button
                                                  ...
                                                  class="flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full text-black/70 hover:text-white hover:bg-black/20 transition"
                                              >
                              ...
                              {props.editLanguages.length < SUPPORTED_LANGUAGES.length && (
                                  <button
                                      ...
                                      class="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#DC9E53] text-[#45AF95] hover:bg-white/90 transition transform hover:scale-110"
                                  >
                          ...
                          {props.editLanguages[props.selectedLanguageIndex] ? (
                              <div class="dropdown dropdown-hover w-full">
                                  <div
                                      tabindex="0"
                                      role="button"
                                      class="w-full px-3 py-3 sm:px-5 sm:py-4 text-sm sm:text-lg border-2 border-[#DC9E53]/30 rounded-xl bg-white/10 backdrop-blur-sm hover:border-white/50 transition cursor-pointer flex items-center gap-2 sm:gap-3"
                                  >
                                      ...
                                      <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#DC9E53]/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          ...
                                          {(lang) => (
                                              <li class="list-none">
                                                  <a
                                                      ...
                                                      class={`flex items-center gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 rounded-lg hover:bg-[#45AF95]/10 transition cursor-pointer text-color-dark no-underline text-sm sm:text-base ${
                              ...
                              <div class="dropdown dropdown-hover w-full">
                                  <div
                                      tabindex="0"
                                      role="button"
                                      class="w-full px-3 py-3 sm:px-5 sm:py-4 text-sm sm:text-lg border-2 border-white/30 rounded-xl bg-white/10 backdrop-blur-sm hover:border-white/50 transition cursor-pointer flex items-center gap-2 sm:gap-3"
                                  >
                ...
                <div class="flex gap-2 sm:gap-3 flex-wrap">
                    <Show when={props.languages && props.languages.length > 0}
                          fallback={<span class="text-black/60 italic text-base sm:text-lg">Aucune langue renseignée</span>}>
                        <For each={props.languages}>
                            {(lang) => (
                                <div class="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-[#DC9E53]/20 backdrop-blur-sm rounded-full">
                                    <span class={`fi fi-${LANGUAGE_TO_FLAG[lang]}`}></span>
